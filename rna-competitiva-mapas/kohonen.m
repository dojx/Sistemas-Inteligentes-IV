...
a = 0.25; % Radio de vecindad

eta = 0.1; % Factor de aprendizaje

w = randn(P, M); % Matriz de pesos
d = zeros(M, 1); % Vector para guardar valores de h
ys = zeros(M, K); % Matriz para guardar salidas de la red

epocas = 100; % Numero de epocas o iteraciones

for g = 1 : epocas
    for i = 1 : K
        % Calcular distancia entre nodos de entrada a la neuronas
        for j = 1 : M
            d(j) = 0;
            for k = 1 : P
                d(j) = d(j) + (x(k, i) - w(k, j))^2;
            end
        end
